...
public class ConfiguratorCoolerWarningTest
{
    public static IEnumerable<object[]> CompatibleComponentsData
    {
        get
        {
            var supportedFrequencies = new List<double>
            {
                ConstantsComputer.RAMFrequency1,
                ConstantsComputer.RAMFrequency2,
                ConstantsComputer.RAMFrequency3,
            };
            var commonSocket = new ProcessorSocket(ConstantsComputer.Socket1);
        ...
        WiFiAdapter wifiAdapter)
    {
        ...
        BuildMessage? warningMessage = status.Messages.FirstOrDefault(m => m.MessageType == StatusType.WithWarnings && m.MessageText == "Cooling system TDP is too small");
